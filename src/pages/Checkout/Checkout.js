...
const Checkout = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async ({firstName, lastName, email, cc,address1,address2,city,country,province,zipCode,phoneNumber}) => {
           ...
           const shippingInformation = {
            ...
            console.log(postInvoiceResponse)
    ...
    return (
        <div className="Checkout">
            <form className="Checkout__Separator" onSubmit={formik.handleSubmit}>
                ...
                <input id="email" name="email" type="email"  onChange={formik.handleChange} value={formik.values.email} className="Checkout__Input Checkout__Mail" placeholder="Correo Electrónico"/>
                ...
                <div className="Checkout__Flex">
                    <div>
                    <input id="firstName" name="firstName" type="text" onChange={formik.handleChange} value={formik.values.firstName} className="Checkout__Input Checkout__Name" placeholder="Nombre" />
                    {formik.errors.firstName ? <div className="Checkout__Error">{formik.errors.firstName}</div> : null}
                    </div>
                    
                    <div>
                        <input id="lastName" name="lastName" type="text" onChange={formik.handleChange} value={formik.values.lastName} className="Checkout__Input"placeholder="Apellido" />
                        {formik.errors.lastName ? <div className="Checkout__Error">{formik.errors.lastName}</div> : null}
                    </div>
                </div>
                <div>
                    <input  id="cc" name="cc" type="number" onChange={formik.handleChange} value={formik.values.cc}  className="Checkout__Input Checkout__CC" placeholder="Cedula" />
                    {formik.errors.cc ? <div className="Checkout__Error">{formik.errors.cc}</div> : null}
                </div>
                <div>
                <input  id="address1" name="address1" type="text" onChange={formik.handleChange} value={formik.values.address1} className="Checkout__Input Checkout__CC" placeholder="Dirección" />
                {formik.errors.address1 ? <div className="Checkout__Error">{formik.errors.address1}</div> : null}
                </div>
                <div>
                <input  id="address2" name="address2" type="text" onChange={formik.handleChange} value={formik.values.address2} className="Checkout__Input Checkout__CC" placeholder="Apartamento, local, etc." />
                </div>
                <div>
                <input  id="city" name="city" type="text" onChange={formik.handleChange} value={formik.values.city}  className="Checkout__Input Checkout__CC" placeholder="Ciudad" />
                {formik.errors.city ? <div className="Checkout__Error">{formik.errors.city}</div> : null}
                </div>
                
                <div className="Checkout__Flex">
                    <div className="Checkout__Country">
                    <select className="Checkout__Input Checkout_Location" name="country" id="country" onChange={formik.handleChange} value={formik.values.country} >
                    ...
                    <select className="Checkout__Input Checkout_Location" name="province" id="province" onChange={formik.handleChange} value={formik.values.province}>
                        <option value="" label="Departamento"/>
                        <option value="Cali" label="Cali"/>
                        <option value="Valle del Cauca" label="Valle del Calle" />
                    ...
                    <div>
                        <input id="zipCode" name="zipCode" onChange={formik.handleChange} value={formik.values.zipCode} type="number" className="Checkout__Input" placeholder="Código Postal"/>
                        {formik.errors.zipCode ? <div className="Checkout__Error">{formik.errors.zipCode}</div> : null}
                    </div>
                    
                </div>
                <div className="Checkout__Phone">
                <input  id="phoneNumber" name="phoneNumber" type="number" onChange={formik.handleChange} value={formik.values.phoneNumber} className="Checkout__Input Checkout__CC" placeholder="Teléfono" />
                {formik.errors.phoneNumber ? <div className="Checkout__Error">{formik.errors.phoneNumber}</div> : null}
                </div>
                
                <button type="submit" className="Checkout_Buy">Comprar</button>
            </form>
            <form method="post" action="https://sandbox.checkout.payulatam.com/ppp-web-gateway-payu/">
                <input name="merchantId"    type="hidden"  value="508029"/>
                <input name="accountId"     type="hidden"  value="512321"/>
                <input name="description"   type="hidden"  value="COMPRAS EN ISSADU.COM"/>
                ...
                <input name="responseUrl"    type="hidden"  value="https://issadu.com./web/payu/confirmation"/>
                <input name="confirmationUrl"    type="hidden"  value="https://issadu.com./web/payu/confirmation"/>
